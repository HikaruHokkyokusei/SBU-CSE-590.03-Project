...
verus! {
    ...
    // Now, we will write the actual safety specification
    spec fn safety(library: Library) -> bool {
        forall |book1: Book, book2: Book| #![auto]
            library.dom().contains(book1) &&
            library.dom().contains(book2) &&
            library.index(book1) != Card::Shelf &&
            book1 != book2
            ==> library.index(book1) != library.index(book2)
    }
    // This just happens to be a very strong specification, such that even without writing any explict proof,
    // the verus is able to prove our proof on the system.
